...
public class TestUpdateAddress {
	...
	public void testAddEmployee() throws Exception {
		...
		when(addressService.update(1L, addressRequestStub)).thenReturn(Optional.of(addressResponse));

		Gson GSON = new Gson();

		mockMvc.perform(MockMvcRequestBuilders.put("/petz/v1/address/1")
				.contentType(MediaType.APPLICATION_JSON)
				.content(GSON.toJson(addressRequestStub))
				.accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());

		verify(addressService).update(1l, addressRequestStub);
